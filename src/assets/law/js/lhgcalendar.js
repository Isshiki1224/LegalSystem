...
J.exend(calendar.prototype, {
    get: function(obj) {
	    var ev = J.gtev(); var e = ev.srcElement || ev.target; var id
		if (obj && obj.id) id = J(obj.id).val(); else id = e.value
		
		if (obj && obj.to) { this.to(obj.to) } else { this.than = this.ty = this.tm = this.td = null }
		
		this.draw(id); this.inu = (obj && obj.id) ? obj.id : e
		this.type = (obj && obj.type) ? obj.type : '-'
		this.time = (obj && obj.time) ? obj.time : false
		
		if (obj && obj.dir == 'right') { this._p.show(e.offsetWidth, 0, e) } else { this._p.show(0, e.offsetHeight, e) }
		
		ev.stopPropagation ? ev.stopPropagation() : (ev.cancelBubble = true)
	},
	
	createbody: function(d, m) {
	    var chead = J(d).crte('div'); var txt
		...
		J('t1', d).hover(function() {
		    ...
		    var d = new Date(); var y = d.getFullYear(); var m = d.getMonth() + 1; var t = d.getDate()
			if (J.calendar.type == '-') { J(J.calendar.inu).val(y + '-' + m + '-' + t) } else { J(J.calendar.inu).val(m + '/' + t + '/' + y) } J.calendar._p.hide(this, J.calendar._p)
		})
	},
	
	draw: function(s) {
		var p; var d = new Date(); var a = []; var fd; var md; var frag; var doc = this._p._doc; var cal = J('#idcal', doc)
	...
	to: function(l) {
	    var a = l.split(','); var v = J('#' + a[0]) ? J.trim(J(a[0]).val()) : a[0]
		if (J('#' + a[0]) && J(a[0]).isnl()) {
		    this.than = this.ty = this.tm = this.td = null; return
		}
		var p = v.split('-'); var d = new Date(p[0], p[1] - 1, p[2]); this.than = a[1]
	...
	setdate: function() {
	    var c = J.calendar; var d = J(this).html(); var time
		if (c.m < 10) c.m = '0' + c.m; if (d < 10) d = '0' + d
		
		time = c.time ? new Date().toLocaleTimeString() : ''
		if (time != '') time = time.length < 8 ? ' 0' + time : ' ' + time
		
		if (c.type == '-') { J(c.inu).val(c.y + '-' + c.m + '-' + d + time) } else { J(c.inu).val(c.m + '/' + d + '/' + c.y + time) } c._p.hide(this, c._p)
	},
	
	over: function() {
		if (this.className != 'tday') J(this).acls('over')
		
		var c = J.calendar; var d = J(this).html()

...
(function($) {
	...
	var msie = (function(w, d) {
		...
		var minWidth = msie < 7 ? 'width' : 'minWidth'
		var selectQueue
		var fixie
		var defaultOptions = {
			// 是否自动计算宽度
	...
	function modifyText(select) {
		...
		var select_ui = $(select).closest('.select_ui')
			var options = select_ui.data('selectuiopts') || {}
			var index = select.selectedIndex
			var labelFn = options.label
			var text = index < 0 ? '' : labelFn.call(select.options[index])
			var textdiv = select_ui.find('.select_text_ui')
			var length = 0
		if (!textdiv.length) {
		...
		if (options.autoWidth) {
			...
			$.each(select.options, function() {
				var text = labelFn.call(this)
					var width = text.match(/[u0000-u00FF]/g)
	...
	$.fn.selectui = function(options) {
		...
		return this.each(function() {
			var modifyTextTimer
				var select = $(this)
				var selectui = select.closest('.select_ui')
